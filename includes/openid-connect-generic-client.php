		...
		if ( ! empty( $expected_issuer ) ) {
			...
			if ( rtrim( $id_token_claim['iss'], '/' ) !== rtrim( $expected_issuer, '/' ) ) {
				...
				return new WP_Error(
					'invalid-iss',
					sprintf(
						__( 'Token issuer does not match expected issuer. Configure the correct issuer in Settings > ICC OpenID Client > Issuer field, or via the OIDC_ISSUER constant.', 'daggerhart-openid-connect-generic' ),
					),
					$id_token_claim
				);
			}
